...
| FR-TH-17 | Auto-generated Google Meet link **at payment-confirm**: when a `PaymentIntent` transitions to `held_in_escrow` (slip-verify or admin manual approve), `GoogleMeetService` is called inline to mint a Calendar event with `conferenceData`/`hangoutsMeet` under a platform service account, persisting `meetingUrl` + `googleCalendarEventId` on the booking and posting a system chat message. Wrapped in try/catch — Calendar API failures log + admin retries via `POST /admin/bookings/:id/regenerate-meet`; payment never rolls back. Postpone-confirm deletes the old event + mints a fresh one on the clone. `GOOGLE_MEET_ENABLED=false` posts a fallback chat message. Frontend: green "เข้าห้องเรียนออนไลน์" button on booking row (disabled until ≤24h to start) + Video icon on schedule grid. Bridges the §11 "use Google Meet externally" gap without implementing in-platform video (FR-TH-08 stays Phase 2). | Must | 1 |
