...
const SUBJECTS = subjectSchema.options;
...
export function QuizRunner() {
  ...
  const questionsQuery = useQuery({
    queryKey: ["quiz", "questions", subject],
    queryFn: () => createApiClient().quiz.questions(subject!),
    enabled: !!subject,
  });
  const questions = questionsQuery.data ?? [];

  const submitMutation = useMutation({
    mutationFn: (next: QuizAnswer[]) => {
      const dto = quizSubmissionSchema.parse({
        subject: subject!,
        answers: next,
      });
      return createApiClient().quiz.submit(dto);
    },
  });

  const error =
    questionsQuery.error?.message ?? submitMutation.error?.message ?? null;
    ...
    return (
      ...
      >
        ...
        <div className="space-y-2">
          <p className="text-slate-500">
            คะแนนพื้นฐานของคุณในวิชา {result.subject}
          </p>
  ...
  if (questionsQuery.isLoading || questions.length === 0) {
    return (
        ...
        <div className="grid grid-cols-1 gap-4">
          {currentQ.options.map((opt, i) => (
            <button
              ...
              className="w-full p-4 rounded-2xl border border-slate-200 text-left hover:border-indigo-600 hover:bg-indigo-50 transition-all group flex items-center justify-between disabled:opacity-50"
            >
