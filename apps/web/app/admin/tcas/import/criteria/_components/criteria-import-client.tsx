...
export function CriteriaImportClient({ accessToken }: Props) {
  const api = createApiClient({ accessToken });
  const [filename, setFilename] = useState<string>("");
  const [preview, setPreview] = useState<TcasCriteriaPreviewResponse | null>(
    null,
  );
  ...
  async function handleFile(file: File) {
    ...
    try {
      const res = await api.admin.tcas.previewCriteria(file);
      setPreview(res);
    } catch (e) {
  ...
  async function handleCommit() {
    ...
    setError(null);
    try {
      const res = await api.admin.tcas.commitCriteria(preview.uploadId);
      setCommitResult(res);
      setPreview(null);
    } catch (e) {
      setError((e as Error).message);
      ...
      {!preview && !commitResult && (
        <label
          ...
          className={cn(
            ...
            previewing
              ? "border-indigo-400 bg-indigo-50/60 animate-pulse"
              : "border-slate-300 bg-white hover:border-indigo-300",
          )}
        >
          <Upload size={28} className="mx-auto text-indigo-500 mb-3" />
          <p className="font-bold text-slate-700">
            {previewing
              ? "กำลังประมวลผล..."
              : "เลือกไฟล์ CSV (เกณฑ์การรับเข้า)"}
          </p>
          <p className="text-xs text-slate-400 mt-1">
            สูงสุด 5 MB • ดู template ด้านบนสำหรับโครงสร้างคอลัมน์
          </p>
          <input
            id="criteria-file"
            type="file"
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                ทั้งหมด {preview.summary.total} แถว • ใหม่{" "}
                {preview.summary.new} • อัปเดต {preview.summary.update} • ไม่เปลี่ยน{" "}
                {preview.summary.unchanged} • ผิดพลาด {preview.summary.error}
              </p>
            </div>
